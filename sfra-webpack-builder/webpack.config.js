...
class WebpackBundle {
        ...
        if (fileType === "js") {
            glob.sync(srcJSPath)
                .filter((source) => {
                    ...
                    if (
                        sfraBuilderConfig.excludeJS &&
                        sfraBuilderConfig.excludeJS[cartridge] &&
                ...
                .forEach((source) => {
                    ...
                    const sourceName = path.basename(source);
                    const outputFile = path.join(sourceRelativePath, sourceName).split(path.sep).join(path.posix.sep);
                    files[outputFile] = source;
                });
        }
        ...
        if (fileType === "jsx") {
            ...
            glob.sync(srcJSXPath)
                ...
                .forEach((source) => {
                    ...
                    const sourceName = path.basename(source);
                    const outputFile = path
                        .join(sourceRelativePath.replace("jsx", "js"), sourceName.replace(".jsx", ".js"))
                        .split(path.sep)
                        .join(path.posix.sep);
                    files[outputFile] = source;
                });
    ...
    static getPlugins(cartridge, fileType, env) {
        var plugins = [];
        if (fileType === "copy" && sfraBuilderConfig.copyConfig && sfraBuilderConfig.copyConfig[cartridge]) {
            plugins.push(
        ...
        if ((fileType === "js" || fileType === "jsx") && env.useLinter) {
            plugins.push(
                new ESLintWebpackPlugin({
                    ...
                    exclude: ["node_modules", sfraBuilderConfig.sfraFolderName || "storefront-reference-architecture"],
                    fix: sfraBuilderConfig.lintConfig && sfraBuilderConfig.lintConfig.eslintFix,
                }),
            );
        }
        if ((fileType === "scss" || fileType === "jsx") && env.useLinter) {
            plugins.push(
                new StyleLintWebpackPlugin({
                    files: `${cartridge}/cartridge/client`,
                    exclude: ["node_modules", sfraBuilderConfig.sfraFolderName || "storefront-reference-architecture"],
                    fix: sfraBuilderConfig.lintConfig && sfraBuilderConfig.lintConfig.stylelintFix,
                }),
            );
        }
        if (fileType === "scss") {
            plugins.push(
                new MiniCssExtractPlugin({
                    filename: (pathData) => pathData.chunk.name.replace(/\.scssjs$/, ".css"),
                }),
        ...
        if (fileType === "jsx") {
            plugins.push(
                new MiniCssExtractPlugin({
                    filename: (pathData) => pathData.chunk.name.replace("/js/", "/css/").replace(/\.js$/, ".css"),
                }),
                new RemoveEmptyScriptsPlugin({ remove: /\.scssjs$/ }),
            );
        }
        if (env.livereload && (fileType === "js" || fileType === "jsx" || fileType === "scss")) {
            plugins.push(
    ...
    static bundleCartridge(env = {}, cartridge, fileType) {
        ...
        let ruleSet = webpackHelper.buildRuleSet(process.env.PWD, cartridge, env, fileType);
...
module.exports = (env) => {
    ...
    console.log(chalk.magenta.bold(` 📦 Clean, Copy and Bundle Cartridges 📦\n`));
