...
describe("scanEntryPoints()", () => {
  it("should return an entry point for JS and SCSS", () => {
    ...
    const jsFile = Object.entries(jsEntryPoint)[1][1];
    const jsFileExclude = Object.entries(jsEntryPoint)[0][1]
    const scssFile = Object.entries(scssEntryPoint)[0][1];
    let fixtureJSFile = "default/js/test.js";
    let fixtureJSfilesToBeExcluded = "filesToBeExcluded.js"
    let fixtureSCSSFile = "default/scss/test.scss";
    assert(jsFile.includes(fixtureJSFile));
    assert(jsFileExclude.includes(fixtureJSfilesToBeExcluded));
...
describe("bundleCartridge()", () => {
  ...
  it("should createOutputfiles for js/css", () => {
    let sfraBuilderConfig = require("../../webpackHandling/fixture_sfraBuilderConfig");
    ...
    exec("npm run test:fixture", (err) => {
      ...
      try {
        ...
        jsFilesExclude = sfraBuilderConfig.excludeJS[Object.keys(sfraBuilderConfig.excludeJS)[0]];
      ...
      assert.isNotTrue(jsFiles.includes(jsFilesExclude)); // Js files in excludes config should not be available output.
    });
  });
});
